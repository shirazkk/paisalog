...
export async function POST(request: NextRequest) {
  try {
    ...
    if (profileError || !profile.household_id) {
      return NextResponse.json({ error: 'You must join a household before logging transactions.' }, { status: 400 })
    }
    ...
    if (!dadProfile || !momProfile) {
      return NextResponse.json({
        error: 'Your partner needs to link their account using the invite code before you can log transactions.'
      }, { status: 400 })
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (profileError || !profile.household_id) {
      return NextResponse.json({ error: 'No household joined yet' }, { status: 400 })
    }
    ...
    console.log('[API Debug] Fetching transactions for household:', householdId, 'Category:', category, 'Month:', month)
    ...
    if (month && month !== 'all') {
      ...
      console.log('[API Debug] Applying date filter:', startOfMonth, 'to', endOfMonth)
