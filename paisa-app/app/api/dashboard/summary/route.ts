...
export async function GET(request: NextRequest) {
  try {
    ...
    // Calculate sums
    const thisMonthTotal = thisMonthTxns.reduce((acc, curr) => acc + Number(curr.amount), 0)
    const lastMonthTotal = lastMonthTxns.reduce((acc, curr) => acc + Number(curr.amount), 0)
    const allTimeTotal = allTimeTxns.reduce((acc, curr) => acc + Number(curr.amount), 0)
